...
export default function MainProses() {
  ...
  useEffect(() => {
    const aksesDataFunc = async () => {
      await axios.get(`${config.base_url}/kontrak?pemilik=${JSON.parse(localStorage.getItem('user_data')).id}&&lokasi_ne=${JSON.parse(localStorage.getItem('user_data')).id}`)
        .then(res => {
          setData(res.data)
        }).catch(err => {
